...
import {
  ...
  formatOracleFormPath,
  logging,
  multiSortBy,
  oracleFormNameToTitle,
  removeExt,
  removeTrailingSlash,
  removeUnderscores,
  replaceBackslash,
...
export async function generator(config: Config) {
  ...
  component: () => <div>Hello ${escapedRoutePath}!</div>
      ...
      // Plexxis
        ...
        if (node.oracleFormTitle) {
          replaced.replace(
            /title:\s*['"][^'"]*['"]/,
            `title: "${node.oracleFormTitle}"`,
          )
        }
      } else {
        replaced = routeCode
    ...
    sortedRouteNodes
      .map((node) => {
        ...
        return [
          ...
          ${[
            // Plexxis
            `id: '${node.oracleFormName ? formatOracleFormPath(node.path) : node.path}'`,
            !node.isNonPath
              ? `path: '${node.oracleFormName ? formatOracleFormPath(node.cleanedPath) : node.cleanedPath}'`
              : undefined,
...
export const createRouteNodesByFullPath = (
  ...
  return new Map(
    routeNodes.map((routeNode) => [
      // Plexxis
      routeNode.oracleFormName
        ? formatOracleFormPath(inferFullPath(routeNode))
        : inferFullPath(routeNode),
      routeNode,
    ]),
  )
}
...
export const createRouteNodesByTo = (
  ...
  return new Map(
    dedupeBranchesAndIndexRoutes(routeNodes).map((routeNode) => [
      // Plexxis
      routeNode.oracleFormName
        ? formatOracleFormPath(inferTo(routeNode))
        : inferTo(routeNode),
      routeNode,
    ]),
  )
...
export const createRouteNodesById = (
  ...
  return new Map(
    routeNodes.map((routeNode) => {
      ...
      return [
        // Plexxis
        routeNode.oracleFormName ? formatOracleFormPath(id) : id,
        routeNode,
      ]
    }),
  )
